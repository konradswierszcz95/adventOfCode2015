...
import pl.konrad.swierszcz.day7.SignalOperationDecoder;
import pl.konrad.swierszcz.day7.SignalTypes;
...
class Solution {
    ...
    private static void insertModifiedValues(List<SignalOperation> operations, Map<String, Integer> operationMap) {
        while (operationMap.size() < operations.size()) {
            ...
            ableToFill.forEach(
                    operation -> {
                        fillSignalOperationData(operation, operationMap.getOrDefault(operation.getFirstInputName(), operation.getFirstInputValue()), operationMap.get(operation.getSecondInputName()));
